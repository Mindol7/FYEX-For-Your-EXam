...
CONFIG:
  ...
  exam_styles:
    ...
    mix:
      ...
      numeric:    {count: 2,  decimals: 3, tolerance: 1e-3, units_required: true, show_rounding_rule: true}
      true_false: {count: 0}
      fill_blank: {count: 0}
  ...
  constraints:
    ...
    essay_area_replaces_short_boxes: true            # (에세이는 좁은 박스 금지)
    short_area_replaces_small_boxes: true            # ★ Short도 박스 여러 개 금지(넓은 영역 사용)
    # === 해설 품질 게이트(추가) ===
    explain_quality_gate:
      mcq_min_words: 80
      short_min_words: 100
      numeric_min_words: 60
      forbid_phrases_in_explain:
        - "표준 정의/특성에 부합합니다"
        - "자명합니다"
        - "상식적으로 맞습니다"
      require_sections:
        mcq: ["정답 근거","오답 분석","출처"]
        numeric: ["문제 해석","공식/대입","계산","반올림/단위"]
  ...
  style:
    ...
    # Short 답안 렌더링 — 단일 넓은 영역
    short_answer:
      answer_box: false                 # ★ 작은 박스 사용하지 않음
      area: true                        # ★ 단일 넓은 영역
      ruled: false                      # 줄노트 선 필요시 true
      area_height_mm: 45                # 영역 높이(필요 시 조절)
      margins_mm: {top: 6, bottom: 6}

    # Essay 답안 렌더링 — 단일 넓은 영역(지난 수정 유지)
    essay_answer:
      area: true
      ruled: false
      area_height_mm: 120
  ...
  STEP_3_EXECUTION:
    ...
    exam_mode:
      ...
      pipeline:
        ...
        - "SHORT 렌더링:
             - constraints.short_area_replaces_small_boxes=true이면
               style.short_answer.area=true 설정을 따라 단일 넓은 답안 영역을 사용
             - ruled=true이면 area_height_mm을 n줄로 균등 분할해 가로줄 표시
             - answer_box=false이므로 작은 박스는 그리지 않음"
        - "ESSAY 렌더링:
             - style.essay_answer.area=true이면 단일 넓은 답안 영역 사용
             - ruled=true이면 줄노트 표시"
        - "MCQ 해설 체크리스트:
             1) 정답 근거(정의/수식/정리 중 ≥1)
             2) 오답 A~D 각각의 반증/틀린 이유
             3) 출처 인용 {explanations.citation.format}"
        - "Numeric 해설: 문제 해석→공식/대입→계산→반올림/단위 점검"
        - "Short/Essay: 모델답안 스케치 + 키워드 표시(볼드/밑줄)"
        - "품질 게이트:
             - 분량 미달/금지어/출처 누락 시 evidence_bank로 보강 후 1회 재시도"
        - "해설/시험 PDF 렌더"
      outputs:
        - {output_files.exam}
        - {output_files.explain}
# =============== STEP FLOW / PIPELINE (END) ===============
